...
internal data class GroupTurnResult(
    ...
    val gameActions: List<GameActionDraft> = emptyList(),
    val gameControls: List<GameRuntimeControlDraft> = emptyList(),
)
...
internal class GroupTurnExecutor(
    ...
    suspend fun runTurn(
        ...
        extraToolIds: List<String> = emptyList(),
        maxSkillCalls: Int = 3,
        ...
        val groupMetas = TaskToolPolicy.select(
            ...
            .let { metas ->
                (metas + extraToolIds.mapNotNull { registry.get(it)?.meta }).distinctBy { it.id }
            }
        ...
        val gameActions = mutableListOf<GameActionDraft>()
        val gameControls = mutableListOf<GameRuntimeControlDraft>()

        repeat(maxSkillCalls + 1) { iteration ->
            if (shouldStop()) return GroupTurnResult(finalText, attachments, gameActions, gameControls)
            ...
            val resp = runCatching {
                ...
                return GroupTurnResult(finalText.ifBlank { if (requireResponse) fallbackGroupReply(role, baseMessages) else "[PASS]" }, attachments, gameActions, gameControls)
            } ?: return GroupTurnResult(finalText.ifBlank { if (requireResponse) fallbackGroupReply(role, baseMessages) else "[PASS]" }, attachments, gameActions, gameControls)
            ...
            (skillResult.data as? GameActionDraft)?.let { draft ->
                gameActions += draft
            }
            (skillResult.data as? GameRuntimeControlDraft)?.let { draft ->
                gameControls += draft
            }
